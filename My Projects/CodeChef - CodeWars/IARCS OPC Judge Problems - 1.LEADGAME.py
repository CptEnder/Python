"""
Created on Sat 17 Apr 01:31 2021
Finished on Sat 17 Apr 01:45 2021
@author: Cpt.Ender
...
Siruseri play against each other. The Manager of Siruseri Sports Club decided to add
his own twist to the game by changing the rules for determining the winner. In his version,
at the end of each round the leader and her current lead are calculated.
Once all the rounds are over the player who had the maximum lead
at the end of any round in the game is declared the winner.
...
indicating the number of rounds in the game. Lines 2,3,...,N+1 describe the scores
of the two players in the N rounds. Line i+1 contains two integer Si and Ti,
...
for _ in range(N):
    in_ = input().split(' ')
    [s, t] = [int(x) for x in in_]
    S += s
    T += t
    currentLead = S - T
    if abs(maxLead) < abs(currentLead):
        maxLead = currentLead
