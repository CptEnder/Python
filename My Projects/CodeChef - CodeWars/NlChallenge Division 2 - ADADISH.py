"""
Created on Sun 08 Nov 11:54 2020
Finished on
@author: Cpt.Ender

https://www.codechef.com/NOV20B/problems/ADADISH

Chef Ada is preparing N dishes (numbered 1 through N).
For each valid i, it takes Ci minutes to prepare the i-th dish.
...
she puts it on one of the burners and after Ci minutes,
removes it from this burner;
the dish may not be removed from the burner before those Ci minutes pass,
because otherwise it cools down and gets spoiled.
Any two dishes may be prepared simultaneously, however,
no two dishes may be on the same burner at the same time.
...
def minTimeRequired(timeList: list):
    tempList = timeList[:]
    firstTimer = max(tempList)
    tempList.remove(firstTimer)
    secondTimer = max(tempList)
    tempList.remove(secondTimer)
    while tempList:
    ...
    return max(firstTimer, secondTimer)
...
for _ in range(T):
    # N = int(input())
    # _in = input().split(' ')
    # C = [int(_in[i]) for i in range(N)]
    C = [2, 3, 4, 5, 6]
    print(minTimeRequired(C))
