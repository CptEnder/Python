"""
Created on Fri 31 Jul 13:23 2020
Finished on
@author: Cpt.Ender
                                  """
...
#
#
# def lerp(var1, var2, weight):
#     # Linearly interpolate between var1 and var2
#     return var1 + weight * (var1 - var2)
#
#
# def fade(t):
#     # Used for smoothing
...
#     # The way we selected the values for the corners in the code above respect this restriction.
#     # If we are in grid cell (0, 0), “valueBottomRight” will be equal to P[P[0+1]+0] = P[P[1]+0].
#     # Whereas in the grid cell (1, 0), “valueBottomLeft” will be equal to P[P[1]+0].
#     # “valueBottomRight” and “valueBottomLeft” are the same. The restriction is respected.
...
#
#     # Interpolate to get only one value
#     u = fade(xf)
#     v = fade(yf)
#     return lerp(lerp(dotBottomLeft, dotTopLeft, v),
...
def smoothstep(t):
    """Smooth curve with a zero derivative at 0 and 1,
    making it useful for interpolating.
    """
    return t * t * (3. - 2. * t)
    # return ((6 * t - 15) * t + 10) * t * t * t
...
def scale(x, start, end, new_start, new_end):
    return new_start + (x - start) * (new_end - new_start) / (end - start)
